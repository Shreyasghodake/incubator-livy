...
object SparkKubernetesApp extends Logging {
  ...
  def init(livyConf: LivyConf, client: Option[KubernetesClient] = None): Unit = {
    ...
    LivyConfTime = livyConf.getTimeAsMs(LivyConf.KUBERNETES_APP_LOOKUP_MAX_FAILED_TIMES)
    appLookupThreadPoolSize = livyConf.getInt(LivyConf.KUBERNETES_APP_LOOKUP_THREAD_POOL_SIZE)
    appLookupMaxFailedTimes = LivyConfTime.milliseconds.toMillis

    SlCheckInterval = livyConf.getTimeAsMs(LivyConf.KUBERNETES_APP_LEAKAGE_CHECK_INTERVAL)
    sessionLeakageCheckInterval = LivyConfTime
